...
class Automation:
    ...
    Methods
    -----
    execute(type: `AutomationType`, bot: `commands.Bot`)
        執行自動排程
    """

    _locks: ClassVar[dict[AutomationType, asyncio.Lock]] = {
        AutomationType.DAILY_REWARD: asyncio.Lock(),
        AutomationType.REALTIME_NOTES: asyncio.Lock(),
    }
    """每個自動排程類型的鎖"""

    @classmethod
    async def execute(cls, type: AutomationType, bot: commands.Bot):
        """執行自動排程
        ...
        bot: `commands.Bot`
            Discord 機器人客戶端
        """
        lock = cls._locks[type]
        if lock.locked():
            return
        await lock.acquire()
        try:
            match type:
                case AutomationType.DAILY_REWARD:
                    await cls._claim_daily_reward(bot)
                case AutomationType.REALTIME_NOTES:
                    await cls._check_realtime_notes(bot)
        except Exception as e:
            sentry_sdk.capture_exception(e)
            LOG.Error(f"自動排程 {type.name} 發生錯誤：{e}")
        finally:
            lock.release()
    ...
    @classmethod
    async def _check_realtime_notes(cls, bot: commands.Bot):
