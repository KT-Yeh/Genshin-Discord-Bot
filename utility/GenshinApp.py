...
class GenshinApp:
    ...
    @generalErrorHandler
    async def getRealtimeNote(
        self, user_id: int, *, schedule=False
        ...
        `Notes`: 查詢結果
        """
        client = await self.__getGenshinClient(
            user_id, update_using_time=(not schedule)
        )
    ...
    async def claimDailyReward(
        self, user_id: int, *, honkai: bool = False, schedule=False
    ) -> str:
        """為使用者在Hoyolab簽到
    ...
    @generalErrorHandler
    async def getSpiralAbyss(
        self, user_id: int, previous: bool = False
    ) -> SpiralAbyssData:
        """取得深境螺旋資訊
    ...
    def parseAbyssOverview(self, abyss: genshin.models.SpiralAbyss) -> discord.Embed:
        ...
        """
        result = discord.Embed(
            description=f'第 {abyss.season} 期：{abyss.start_time.astimezone().strftime("%Y.%m.%d")} ~ {abyss.end_time.astimezone().strftime("%Y.%m.%d")}',
            color=0x6959C1,
        )
    ...
    async def parseNotes(
        ...
        *,
        ...
        # 原粹樹脂
        resin_title = (
            f"{emoji.notes.resin}當前原粹樹脂：{notes.current_resin}/{notes.max_resin}\n"
        )
        ...
        if not shortForm:
            ...
            resin_msg += (
                "獎勵已領\n"
                if notes.claimed_commission_reward == True
                else "**尚未領獎**\n"
        ...
        if notes.max_realm_currency > 0:
            if notes.current_realm_currency >= notes.max_realm_currency:
                ...
                recover_time = (
                    f'{day_msg} {notes.realm_currency_recovery_time.strftime("%H:%M")}'
                )
        ...
        for expedition in notes.expeditions:
            ...
            if expedition.finished:
                ...
                exped_msg += (
                    f'{day_msg} {expedition.completion_time.strftime("%H:%M")}\n'
                )
        ...
        if user != None:
            ...
            embed.set_author(
                name=f"{getServerName(uid[0])} {uid}", icon_url=user.display_avatar.url
            )
        return embed

    async def __getGenshinClient(
        self, user_id: int, *, check_uid=True, update_using_time: bool = True
    ) -> genshin.Client:
        """設定並取得原神API的Client
