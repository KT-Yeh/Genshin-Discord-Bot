...
class ColorTool():
        ...
        for custom_color in custom_colors:
            if len(custom_color) != 0:
                ...
                if isinstance(custom_color[0], int):
                    ...
                    color = self.RGB(custom_color[0], custom_color[1],
                                     custom_color[2], font)
                else:
    ...
    def RGB(self, Red: int = 255, Green: int = 255, Blue: int = 255, font: bool = True) -> str:
        ...
        Returns:
            str: ANSI轉義序列
        """
        if ((isinstance(Red, int) and Red >= 0 and Red <= 255)
...
class LogTool(ColorTool):
    ...
    def Cog(self, id: str, name: str = '', enabled: bool = True):
        if enabled:
            return (
                ...
                if name != '' else
                f'{self._ORANGE_RED}{id}{self.RESET}'
            )
        else:
            return (
    ...
    def CmdResult(self,
        ...
        if isinstance(ctx, commands.Context):
            ...
            cmd_name = (
                command_name if command_name != None
                else f'/{ctx.command.name}' if isinstance(ctx.command, discord.app_commands.Command)
                else f'\u200b{ctx.command.name}' if isinstance(ctx.command, discord.app_commands.ContextMenu)
                else '(無相關指令)'
            )
