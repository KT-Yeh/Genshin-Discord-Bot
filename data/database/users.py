...
class User:
    ...
    def __init__(self, *, id: int = None, cookie: str = None, uid: int = None, last_used_time: typing.Union[datetime, str] = None):
        self.id = id
        self.cookie = cookie
        self.uid = uid
...
class UsersTable:
    ...
    async def update(self, user_id: int, *, cookie: str = None, uid: int = None, last_used_time: bool = False) -> None:
        if cookie:
    ...
    async def exist(self, user: User, *, check_uid = True, update_using_time = True) -> typing.Tuple[bool, str]:
        """檢查使用者相關資料是否已保存在資料庫內
        
        ------
        Parameters
        user `database.User`: 使用者
